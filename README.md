# gauge-go
[![Gauge Badge](http://getgauge.io/Gauge_Badge.svg)](http://getgauge.io) [![Build Status](https://snap-ci.com/ezbuy/gauge-go/branch/master/build_image)](https://snap-ci.com/ezbuy/gauge-go/branch/master)
...
#### Example project
A sample project illustrating Gauge features using Golang & selenium webdriver can be found [here](https://github.com/ezbuy/gauge-example-go).
...
import (
	...
	. "github.com/ezbuy/gauge-go/testsuit"
)    
...
```
You can use `T.Fail` or `T.Errorf` functions to mark a step as failure, which are under package `github.com/ezbuy/gauge-go/testsuit`. You can also use any assertion libraries like [testify](https://github.com/stretchr/testify), by passing `testsuit.T` as argument to assertions.
...
There are 3 different types of data stores based on the lifecycle of when it gets cleared. These are present in the `github.com/ezbuy/gauge-go/gauge` package of gauge-go plugin.
...
Custom messages/data can be added to execution reports using the below API from the step implementations. The API is under the package `github.com/ezbuy/gauge-go/gauge` which should be imported.
...
```go
import "github.com/ezbuy/gauge-go/gauge"
